...
public:
	...
	R1370(UART& use_uart)
		: use_uart_(use_uart),
		  uart_interrupt_(use_uart.useUartName(), [&](uint8_t receive_byte){ receive_data_(receive_byte);} )
	{
	}
	R1370(UART& use_uart, const std::function<void(void)>&& calc_odom)
		: use_uart_(use_uart),
		  uart_interrupt_(use_uart.useUartName(),
				  [&](uint8_t receive_byte)
				  {
					receive_data_(receive_byte);
					calc_odom();
				  })
	{
	}
